    ...
    <React.Fragment>
      <Toolbar className={classes.toolbar}>
        ...
        <form onSubmit={handleSearch}>
          <InputBase
            ...
            placeholder={`${translate('common.search')} ${translate('purchaseOrder.productCode')}`}
            inputProps={{ 'aria-label': `${translate('common.search')} ${translate('purchaseOrder.productCode')}` }}
          />
      ...
      <div className={classes.chips}>
        {search && (
          <Chip
            label={search}
            onDelete={cancelSearch}
          />
        )}
        {timeConstraint.map(tc => (
          <Chip
            key={`product.${tc}`}
            label={translate(`product.${tc}`)}
            onDelete={handleFilter(tc)}
          />
        ))}
        {selectedCategories.map(categoryId => (
          <Chip
            key={`product.${categoryId}`}
            label={categories.find(c => c.id === categoryId).nameEn}
            onDelete={handleFilterCat(categoryId)}
          />
        ))}
      </div>
    </React.Fragment>
